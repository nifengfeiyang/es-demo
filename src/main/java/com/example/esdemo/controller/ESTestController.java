...
import com.example.esdemo.vo.ResponseBean;
import com.example.esdemo.vo.User;
...
public class ESTestController {
    ...
    private final static String TYPE ="users";
    ...
    public ResponseBean createIndex(@RequestParam String indexName) {
        try {
            ...
            // 2、设置索引的settings
            createIndexRequest.settings(Settings.builder().put("index.number_of_shards", 5) // 分片数
    ...
    public ResponseBean findIndustryClassList(@RequestBody User user, @RequestParam String indexName,@RequestParam String uid) {
        IndexRequest indexRequest = new IndexRequest(indexName,TYPE,uid);//不指定id会报错
...
//        FuzzyQueryBuilder fuzzyQueryBuilder = QueryBuilders.fuzzyQuery("name", "三");//模糊查询
    ...
    public ResponseBean testESUpdate(@RequestParam String id, @RequestParam Double money) {
        UpdateRequest updateRequest = new UpdateRequest("test_es",TYPE, id);
